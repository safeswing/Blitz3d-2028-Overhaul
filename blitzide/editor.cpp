...
void Editor::OnPaint() {
	...
	// Fill the structural divider column with matching dark canvas slate
	CBrush br(RGB(30, 30, 30));
	CRect tr(x, y, x + w, y + h);
	dc.FillRect(&tr, &br);

	// Draw a single flat, dark subtle border instead of a 3D white highlight edge
	CRect dr(x + 2, y, x + 3, y + h);
	CBrush borderBrush(RGB(50, 50, 50));
	dc.FillRect(&dr, &borderBrush);
}
